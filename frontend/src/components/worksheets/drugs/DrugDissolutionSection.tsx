...
import SamplePreparationDissoDetail from "../../sub-components/drugs/SamplePreparationDissoDetail";
import CalculationDetailDisso from "../../sub-components/drugs/CalculationDetailDisso";
import DissoMediaPreparationDetail from "../../sub-components/drugs/DissoMediaPreparationDetail";
import { Plus, Target } from "../../shared/WorksheetUiHelpers";
import WorksheetFileAttacher from "../../shared/WorksheetFileAttacher";

interface DrugDissolutionSectionProps {
  parameterId: number;
  isActive: boolean;
  ...
  handleAddStandardPreparationDisso: (parameterId: number) => void;
  handleRemoveStandardPreparationDisso: (...args: any[]) => void;
  handleStandardPreparationDissoStepChange: (...args: any[]) => void;

  handleRemoveSamplePreparationDisso: (...args: any[]) => void;
  handleSamplePreparationDissoStepChange: (...args: any[]) => void;
  handleDissoMediaStepChange: (...args: any[]) => void;
  handleRemoveDissoMedia: (...args: any[]) => void;

  handleAddCalculationDisso: (parameterId: number) => void;
  handleRemoveCalculationDisso: (...args: any[]) => void;
  handleCalculationDissoFieldChange: (...args: any[]) => void;

  onComplete: () => void;
  onUnlock: () => void;
}

const DrugDissolutionSection: React.FC<DrugDissolutionSectionProps> = ({
  parameterId,
  isActive,
  isPreparationLocked,
  ...
  const safeMediaPreparations = Array.isArray(mediaPreparations) ? mediaPreparations : [];
  const safeStandardPreparations = Array.isArray(standardPreparations)
    ? standardPreparations
    : [];
  const safeSamplePreparations = Array.isArray(samplePreparations)
    ? samplePreparations
    : [];
  const safeCalculations = Array.isArray(calculations) ? calculations : [];
  const safeAssignedStandards = Array.isArray(assignedStandards)
    ? assignedStandards
    : [];
    ...
    <>
      {isActive && (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-8">
                                  ...
                                  <div className="px-4 py-1 bg-gradient-to-r from-emerald-50 to-emerald-50 border border-emerald-200 rounded-full shadow-sm">
                                    <span className="text-xs font-bold text-emerald-800">
                                      {safeMediaPreparations
                                        .length +
                                        (
                                          safeStandardPreparations
                                        ).length +
                                        (
                                          safeSamplePreparations
                                        ).length +
                                        (
                                          safeCalculations
                                        ).length}
                                ...
                                <div className="mb-8">
                                  <div className="flex items-center justify-between mb-4 px-2">
                                    <h3 className="text-lg font-bold text-emerald-800 flex items-center gap-2.5 tracking-tight">
                                      ...
                                      Media, Standard & Sample Preparations for
                                      Dissolution
                                    </h3>
                                    <button
                                      onClick={() =>
                                        ...
                                        <div className="flex items-center gap-3 px-5 py-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                                          ...
                                          <div className="flex-1">
                                            <p className="text-sm font-semibold text-emerald-800">
                                              Dissolution Preparation Completed
                                            </p>
                              ...
                              {safeStandardPreparations.length > 0 &&
                                safeSamplePreparations.length > 0 &&
                                completedAt && (
                                  <div
                                    className={
                                      isFullyLocked
                                      ...
                                      <div className="relative p-6 rounded-xl border border-emerald-200/30 bg-white/50 backdrop-blur-sm shadow-lg">
                                        <div className="flex items-center justify-between mb-6 px-2">
                                          ...
                                          <motion.button
                                            ...
                                            whileHover={{ scale: 1 }}
                                            whileTap={{ scale: 1 }}
                                            className="flex items-center gap-1.5 p-2.5 bg-gradient-to-r from-emerald-600 to-emerald-900 text-white font-semibold rounded-xl hover:from-emerald-700 hover:to-emerald-800 transition-all duration-200 shadow-lg hover:shadow-xl text-xs"
                                          >
                                            <Plus className="w-4 h-4" />
                                            Add Calculation
                                          </motion.button>
                                            ...
                                            >
                                              ...
                                              <div className="relative z-10">
                                                ...
                                                <p className="font-semibold text-base text-emerald-800 mb-1">
                                                  No dissolution safeCalculations added yet
                                                </p>
...
export default DrugDissolutionSection;
