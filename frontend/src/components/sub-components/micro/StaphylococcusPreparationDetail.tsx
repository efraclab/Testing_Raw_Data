...
import type { StaphylococcusInoculationRow } from "../../../preparation_models/micro/StaphylococcusInoculationRow";
import type { StaphylococcusBiochemicalRow } from "../../../preparation_models/micro/StaphylococcusBiochemicalRow";
import type { StaphylococcusPreparation } from "../../../preparation_models/micro/StaphylococcusPreparation";

const makeDefaultInoculationRows = (): StaphylococcusInoculationRow[] => [
    {
...
const makeDefaultBiochemicalRows = (): StaphylococcusBiochemicalRow[] => [
    {
        testName: "Growth in Brain Heart Infusion Broth",
        analysisStarted: "",
        analysisCompleted: "",
...
const makeEmptyInoculationRow = (): StaphylococcusInoculationRow => ({
    medium: "",
...
export const createDefaultStaphylococcusPreparation = (index: number): StaphylococcusPreparation => ({
    id: Date.now() + index,
    label: `Staphylococcus Preparation ${index + 1}`,
...
const CirclesIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
interface StaphylococcusPreparationDetailProps {
    preparation: StaphylococcusPreparation;
    onChange: (updated: StaphylococcusPreparation) => void;
    onRemove?: () => void;
    isLocked?: boolean;
}
...
const StaphylococcusPreparationDetail: React.FC<StaphylococcusPreparationDetailProps> = ({
    preparation,
    onChange,
    onRemove,
    isLocked = false,
    ...
    const update = useCallback(
        <K extends keyof StaphylococcusPreparation>(key: K, value: StaphylococcusPreparation[K]) => {
    ...
    const updateInoculationRow = (idx: number, field: keyof StaphylococcusInoculationRow, value: string) => {
        onChange({
            ...preparation,
    ...
    const updateBiochemicalRow = (idx: number, field: keyof StaphylococcusBiochemicalRow, value: string) => {
        onChange({
            ...preparation,
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p
                            ...
                            >{preparation.label}</p>
                        <p className="text-xs text-emerald-600 font-medium">Isolation and Detection of Staphylococcus aureus</p>
                    </div>
                </div>
                {onRemove && !isLocked && (
                    <motion.button
            ...
            {/* ══════════════════════════════════════════════
                SECTION 2 — Selective Enrichment Protocol
            ══════════════════════════════════════════════ */}
            <SectionCard>
                <SectionHeader
                    icon={<CirclesIcon className="w-4 h-4 text-white" />}
                    title="Selective Enrichment Protocol"
                    subtitle="Mannitol Salt Agar for Staphylococcus aureus isolation"
                />
                <div className="p-5">
                    <div className="flex items-start gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                        <span className="text-emerald-500 text-base leading-none mt-0.5">🧬</span>
                        <div className="text-xs text-emerald-800 leading-relaxed">
                            <strong>Primary Enrichment:</strong> Inoculate sample in Soya bean Casein Digest Medium at 30-35°C for 24 hours.
                            <br />
                            <strong>Selective Plating:</strong> From incubated SCDM, streak on Mannitol Salt Agar and incubate at 30-35°C for 18-72 hours.
                            <br />
                            <strong>Characteristic Colonies:</strong> Look for yellow/white colonies surrounded by yellow zones (mannitol fermentation).
                        </div>
                    </div>
                </div>
            </SectionCard>
            ...
            <SectionCard>
                <SectionHeader
                    ...
                    subtitle="Sequential inoculation steps for Staphylococcus aureus isolation"
                    badge={
                        !isLocked && (
                            <motion.button
            ...
            <SectionCard>
                <SectionHeader
                    ...
                    subtitle="Biochemical identification from Mannitol Agar colonies"
                    badge={
                        !isLocked && (
                            <motion.button
                                whileHover={{ scale: 1.03 }}
                ...
                <div className="p-3">
                    <div className="flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-800">
                        <strong>Note:</strong> From the growth on Mannitol Agar, colonies are inoculated in Nutrient Agar and Brain Heart Infusion Broth for identification tests.
                    </div>
                </div>
                <div className="overflow-x-auto">
            ...
            <SectionCard>
                <SectionHeader
                    ...
                    title="Final Result"
                    subtitle="Overall compliance determination for Staphylococcus aureus detection"
                />
                <div className="p-5">
                    <textarea
...
export default StaphylococcusPreparationDetail;