...
// ─── Re-export aliased types for TYMC ────────────────────────────────────────
export type TYMCPreparation = TVCWaterPreparation;
export type TYMCObservationRow = TVCWaterObservationRow;
...
const makeDefaultObservationRows = (): TYMCObservationRow[] => [
    { replicate: "", dilutionExponent: UNSET_DILUTION, dilutionCount: "", blank: "" },
    { replicate: "", dilutionExponent: UNSET_DILUTION, dilutionCount: "", blank: "" },
];

export const createDefaultTYMCPreparation = (index: number): TYMCPreparation => ({
    id: Date.now() + index,
    label: `TYMC Preparation ${index + 1}`,
    inoculationVolume: "",
    incubationTemp: "",
    incubationTempUnit: "℃",
    incubationTime: "",
...
interface TYMCPreparationDetailProps {
    preparation: TYMCPreparation;
    onChange: (updated: TYMCPreparation) => void;
    onRemove?: () => void;
    isLocked?: boolean;
}
...
const TYMCPreparationDetail: React.FC<TYMCPreparationDetailProps> = ({
    preparation,
    onChange,
    onRemove,
    isLocked = false,
    ...
    const update = useCallback(
        <K extends keyof TYMCPreparation>(key: K, value: TYMCPreparation[K]) => {
            onChange({ ...preparation, [key]: value });
        },
        [preparation, onChange],
    );

    const rows: TYMCObservationRow[] =
    ...
    const updateRow = (idx: number, field: keyof TYMCObservationRow, value: string | number) => {
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-emerald-600 font-medium">Total Yeast And Mould Count</p>
                    </div>
                </div>
                {onRemove && !isLocked && (
                    <motion.button
...
export default TYMCPreparationDetail;