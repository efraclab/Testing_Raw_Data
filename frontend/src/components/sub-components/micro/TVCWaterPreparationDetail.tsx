...
const ALL_SPECIAL_VALUES = [...TNTC_VALUES, LESS_THAN_ONE];
...
// dilutionExponent stored as null = "no selection yet"
const UNSET_DILUTION = null as unknown as number;

const makeDefaultObservationRows = (): TVCWaterObservationRow[] => [
    { replicate: "", dilutionExponent: UNSET_DILUTION, dilutionCount: "", blank: "" },
    { replicate: "", dilutionExponent: UNSET_DILUTION, dilutionCount: "", blank: "" },
];

export const createDefaultTVCWaterPreparation = (index: number): TVCWaterPreparation => ({
    id: Date.now() + index,
    label: `TVC (Water) Preparation ${index + 1}`,
    inoculationVolume: "",
    incubationTemp: "",
    incubationTempUnit: "℃",
    incubationTime: "",
...
// ─── Dilution Combo Input — numeric input + TNTC/> 300/< 1 dropdown ──────────────
...
const DilutionComboInput: React.FC<{
    ...
    const isSpecial = ALL_SPECIAL_VALUES.includes(value);
...
const CustomDilutionDropdown: React.FC<{
    ...
    return (
        <div ref={ref} className="relative">
            {/* Trigger — styled to sit inside the emerald header */}
            <button
                type="button"
                disabled={disabled}
            ...
            {/* Dropdown panel — renders below, never clipped by overflow-hidden on the table wrapper */}
            <AnimatePresence>
                {open && (
                    <motion.div
                        initial={{ opacity: 0, y: -4, scale: 0.97 }}
                        ...
                        exit={{ opacity: 0, y: -4, scale: 0.97 }}
                        transition={{ duration: 0.12 }}
                        // position: fixed so it escapes the overflow-x-auto / overflow-hidden parents
                        style={{ position: "fixed", zIndex: 9999 }}
                        className="mt-1 min-w-[110px] bg-white border border-emerald-200 rounded-xl shadow-2xl overflow-hidden"
                        ref={(el) => {
                            // Align dropdown under the trigger button
                            if (el && ref.current) {
                                const btn = ref.current.querySelector("button");
                                if (btn) {
                                    const r = btn.getBoundingClientRect();
...
interface TVCWaterPreparationDetailProps {
    preparation: TVCWaterPreparation;
    onChange: (updated: TVCWaterPreparation) => void;
    onRemove?: () => void;
    isLocked?: boolean;
}
...
const TVCWaterPreparationDetail: React.FC<TVCWaterPreparationDetailProps> = ({
    preparation,
    onChange,
    onRemove,
    isLocked = false,
    ...
    const update = useCallback(
        <K extends keyof TVCWaterPreparation>(key: K, value: TVCWaterPreparation[K]) => {
    ...
    const rows: TVCWaterObservationRow[] =
    ...
    // ── Shared dilution: sync both rows ─────────────────────────────────────
    ...
    const updateRow = (idx: number, field: keyof TVCWaterObservationRow, value: string | number) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-emerald-600 font-medium">Total Viable Count – Water Sample</p>
                    </div>
                </div>
                {onRemove && !isLocked && (
                    <motion.button
            ...
            <SectionCard>
                ...
                />
                {/* overflow-visible so the fixed-position dropdown can escape */}
                    ...
                    <table className="w-full text-sm border-collapse min-w-[640px]">
                        <thead>
                            <tr className="bg-emerald-700 text-white">
                                ...
                                {/* Dilution — label + dropdown side by side inside the header */}
                                <th
                        ...
                        <tbody>
                            ...
                            <tr className="border-b border-emerald-100 bg-white hover:bg-emerald-50/40 transition-colors">
                                ...
                                {/* Dilution — rowSpan=2, two stacked colony-count inputs */}
...
export default TVCWaterPreparationDetail;