...
interface Props {
  ...
  onUpdate: (updated: CalculationIcpoesWater) => void;
  onRemove: () => void;
  isLocked?: boolean;
}
...
const CalculationDetailIcpoesWater: React.FC<Props> = ({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  // Core formula — all units normalised before arithmetic
  // Result (mg/L) = (Sample [ppb] − Blank [ppb]) × V1 [mL] × DF1 × DF2 × DF3 / 1000
  const computeResult = (
  ...
  const handleField = (field: keyof CalculationIcpoesWater, value: string | null) => {
    if (isLocked) return;
        ...
        <div className="relative flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-4 flex-1 cursor-pointer select-none" onClick={() => setIsExpanded(!isExpanded)}>
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100">ICP-OES (Water) — Content calculation</p>
            </div>
          </div>
            ...
            <div className="p-6 bg-gradient-to-b from-gray-50 to-white space-y-6">
              ...
              <div className="bg-white rounded-lg border-2 border-emerald-200 overflow-hidden">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <p className="text-xs text-center text-gray-600">All values are converted to canonical units (ppb, mL) before calculation. Output unit is fixed at <strong>{RESULT_UNIT}</strong>.</p>
                </div>
              </div>
