...
interface Props {
  ...
  onUpdate: (updated: CalculationIcpmsIchQ3D) => void;
  onRemove: () => void;
  isLocked?: boolean;
}
...
const CalculationDetailIcpmsIchQ3D: React.FC<Props> = ({
  calculation,
  samplePreparations,
  onUpdate,
  onRemove,
  ...
  const handleField = (field: keyof CalculationIcpmsIchQ3D, value: string | null) => {
    if (isLocked) return;
        ...
        <div className="relative flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-4 flex-1 cursor-pointer select-none" onClick={() => setIsExpanded(!isExpanded)}>
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100">ICP-MS (ICH-Q3D) — Content calculation</p>
            </div>
          </div>
